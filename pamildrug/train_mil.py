...
parser.add_argument('--use_probability_attention', action='store_true', default=False,
                    help='Use probability-space attention instead of feature-space (PSMIL-inspired)')
parser.add_argument('--probability_attention_type', type=str, default='true', choices=['true', 'gated', 'full'],
                    help='Type of probability attention: true (aggregate probabilities), gated (simplified), or full (with projections)')
...
def train_single_fold(test_plate: str) -> None:
    ...
    if args.use_sc_mil:
        ...
        print(f"Probability attention: {args.use_probability_attention} (type: {args.probability_attention_type})")
        model = MILEncoder(
            ...
            use_probability_attention=args.use_probability_attention,
            probability_attention_type=args.probability_attention_type
        )
    else:
        ...
        model = AttentionMILModel(
            num_classes=num_classes, num_heads=args.num_heads, dropout=args.dropout, 
            num_channels=args.num_channels, pretrained=args.pretrained,
            use_probability_attention=args.use_probability_attention,
            probability_attention_type=args.probability_attention_type
        )
