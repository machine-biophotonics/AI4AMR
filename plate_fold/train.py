...
if args.run_all_folds:
    ...
    for r in fold_results:
        print(f"Test plate {r.get('config', {}).get('test_plate', '?')}: "
              f"Val Acc: {r.get('results', {}).get('best_val_acc', 0):.2%}, "
              f"Test Acc: {r.get('results', {}).get('test_acc', 0):.2%}")
...
# Get plate info for each sample
train_plates = []
...
def get_combined_weights(plates, labels):
    """Compute combined class + domain weights for batch"""
    weights = []
