#!/usr/bin/env python3
"""
Generate aggregate confusion matrices - multiple versions:
1. Binary: 1 if correct, 0 if wrong (with random baseline)
2. Raw counts: actual prediction counts
3. Percentages: normalized (with random baseline)
"""
...
def plot_binary_cm(cm_sum, labels, title, output_path):
    """Binary confusion matrix: 1 if accuracy > 50%, 0 otherwise"""
    n = len(labels)
    ...
    # Use Blues colormap like the percentage version
...
def plot_percentage_cm(cm_sum, labels, title, output_path, show_annot=True):
    """Percentage confusion matrix with random baseline"""
    n = len(labels)
    import seaborn as sns
...
def main():
    ...
    for level_key, level_name in levels:
        for hier in hierarchies:
            ...
            # Sort labels only when --family flag is used
            if args.family:
                def sort_key(label):
                    base = get_base_gene(label)
