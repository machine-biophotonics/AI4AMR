...
from src.mil_model import AttentionMILModel, MultiCropDataset, get_gene_from_path, extract_well_from_filename
from src.bag_mix import create_bag_mixer, add_bagmix_args, add_psemix_args
from src.mammoth_import import create_mammoth, add_mammoth_args
...
parser.add_argument('--random_neighborhood', action='store_true', default=False, help='Randomize neighborhood size each epoch')
parser.add_argument('--neighborhood_range', type=int, nargs=2, default=[3, 11], help='Range for random neighborhood sizes (min max)')
parser.add_argument('--cycle_neighborhood', action='store_true', default=False, help='Cycle through neighborhood sizes each epoch')
parser.add_argument('--max_positions', type=int, default=None, help='Maximum number of crop positions to sample')
...
parser.add_argument('--use_mildropout', action='store_true', help='Use MIL-Dropout')
parser.add_argument('--mildropout_topk', type=int, default=3, help='Top-k instances to drop')

# PSAMIL arguments
parser.add_argument('--use_psamil', action='store_true', help='Use PSAMIL (Probability-Space MIL)')
parser.add_argument('--use_psamil_alignment', action='store_true', default=True, help='Use probability-space alignment in PSAMIL')
parser.add_argument('--psamil_alignment_alpha', type=float, default=0.5, help='Alignment strength for PSAMIL')
...
with open(os.path.join(BASE_DIR, 'data', 'plate_well_id_path.json'), 'r') as f:
...
def train_single_fold(test_plate):
    ...
    train_dataset = MultiCropDataset(train_paths, train_labels, plate_maps, 
                                  ...
                                  random_neighborhood=args.random_neighborhood,
                                  neighborhood_range=tuple(args.neighborhood_range),
                                  cycle_neighborhood=args.cycle_neighborhood,
...
dropout_ratio=args.bag_mix_dropout,
    ...
    # Add mildropout if enabled
    mildropout = None
    if args.use_mildropout:
        from src.mil_model import Mildropout
        mildropout = Mildropout(topk=args.mildropout_topk)
        print(f"MIL-Dropout: enabled (top-{args.mildropout_topk})")
    
    # Create model: PSAMIL or standard AttentionMILModel
    if args.use_psamil:
        from src.psamil_model import PSAMILModel
        mildropout_config = None
        if args.use_mildropout:
            from src.mildropout import Mildropout
            mildropout_config = {'topk': args.mildropout_topk, 'kernel': 7, 'dropout_rate': 0.5}
        
        model = PSAMILModel(
            num_classes=num_classes,
            num_heads=args.num_heads,
            attention_temp=0.5,
            use_alignment=args.use_psamil_alignment,
            alignment_alpha=args.psamil_alignment_alpha,
            use_mildropout=args.use_mildropout,
            mildropout_config=mildropout_config,
        )
        print(f"PSAMIL Model: alignment={args.use_psamil_alignment}, alpha={args.psamil_alignment_alpha}")
    else:
        model = AttentionMILModel(num_classes=num_classes, num_heads=args.num_heads, mammoth=mammoth, mildropout=mildropout)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in tqdm(train_loader, desc=f'Epoch {epoch}', leave=False):
            # Handle dict (from MultiCropDataset) or tuple
