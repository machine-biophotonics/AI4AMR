#!/usr/bin/env python3
"""
Generate DINOv3 Embeddings for CRISPRi Images at Full Resolution

This script:
1. Loads DINOv3 ViT-L pretrained model
2. Processes full image at original resolution (2720x2720)
3. Extracts CLS token embedding only (1024-dim)
4. Saves embeddings to disk in organized folder structure

Output structure:
    embeddings_full_res/
    ├── P1/
    │   ├── WellA01_Channel.../
    │   │   └── embedding.npy   (1024-dim CLS token)
    │   └── ...
    ├── P2/ ... P6/ ...
    └── metadata.json
"""
...
parser = argparse.ArgumentParser(description='Generate DINOv3 embeddings for CRISPRi images at full resolution')
...
parser.add_argument('--resize', type=int, default=None, help='Resize images to this size (default: use full resolution)')
...
OUTPUT_DIR = os.path.join(BASE_DIR, "embeddings_full_res")
...
class FullImageEmbeddingDataset(Dataset):
    """Dataset that loads full images for embedding extraction."""
    
    def __init__(self, image_paths: List[str], resize: Optional[int] = None):
        ...
        self.original_size = sample.size
        print(f"  Original image size: {self.original_size}")
        if resize:
            print(f"  Resizing to: {resize}x{resize}")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, str, str, str]:
        ...
        return img_tensor, plate, img_name, img_path
...
def extract_and_save_embeddings(model, dataset: Dataset, batch_size: int, 
    ...
    """Extract CLS token embeddings and save to disk."""
    ...
    metadata = {
        ...
        'resize': args.resize,
        'original_size': dataset.original_size if hasattr(dataset, 'original_size') else None,
    ...
    for batch_idx, (batch_images, plates, img_names, img_paths) in enumerate(tqdm(dataloader, desc="Extracting embeddings")):
        batch_size_actual = batch_images.size(0)
        
        batch_images = batch_images.to(device)
        
        with torch.no_grad():
            outputs = model.forward_features(batch_images)
        ...
        for i in range(batch_size_actual):
            ...
            embedding_path = os.path.join(img_dir, "embedding.npy")
            np.save(embedding_path, cls_token[i])
            
            metadata['images'].append({
                'plate': plate,
                'image_name': img_name,
                'image_path': img_path,
                'embedding_dim': cls_token.shape[1],
...
def main():
    print("\n" + "="*60)
    print("DINOv3 Full Resolution Embedding Generation")
    print("="*60)
    print(f"\nSettings:")
    print(f"  Model: DINOv3 {args.model_size}")
    print(f"  Resize: {args.resize if args.resize else 'full resolution'}")
    print(f"  Embedding: CLS token only ({MODEL_CONFIGS[args.model_size]['embed_dim']}-dim)")
    ...
    embed_dim = MODEL_CONFIGS[args.model_size]['embed_dim']
    est_size_gb = total_images * embed_dim * 4 / (1024**3)
    
    print(f"\nTotal: {total_images} images × {embed_dim}-dim = {est_size_gb:.2f} GB")
    ...
    for plate, paths in plate_paths.items():
        ...
        dataset = FullImageEmbeddingDataset(paths, resize=args.resize)
