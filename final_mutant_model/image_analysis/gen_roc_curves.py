...
for any pixel statistics CSV with standard or hm224_ column naming.
"""
...
parser.add_argument('--prefix', default=None,
                    help='Optional column prefix (e.g. hm224_ for matched CSV). '
                         'If set, looks for {prefix}{region}_* columns.')
...
METHOD_LABEL = args.prefix.rstrip('_') if args.prefix else 'raw'

def get_cols(region, df):
    prefixes_to_try = []
    if args.prefix:
        prefixes_to_try.append(f'{args.prefix}{region}')
    prefixes_to_try.append(region)
    for pfx in prefixes_to_try:
        cols = [f'{pfx}_mp_{f}' for f in ALL_FEATS]
        cols = [c for c in cols if c in df.columns]
        if cols:
            return cols
        cols = [f'{pfx}_raw_{f}' for f in ALL_FEATS if f'{pfx}_raw_{f}' in df.columns]
        if cols:
            return cols
    return []
...
for region in REGIONS:
    ...
    if not cols:
        print(f'  Skipping {region}: no columns found')
        continue
    ...
    ax.set_title(f'Within-Plate 5-fold CV — {region} ({METHOD_LABEL})\n'
...
for region in REGIONS:
    cols = get_cols(region, df)
    if not cols:
        print(f'  Skipping {region} cross-plate: no columns found')
        continue
    fig, ax = plt.subplots(figsize=(10, 8))
    all_aucs = []
    ...
    ax.set_xlabel('False Positive Rate', fontsize=13)
    ax.set_ylabel('True Positive Rate', fontsize=13)
    ax.set_title(f'Cross-Plate 4/1/1 Leave-Plate-Out — {region} ({METHOD_LABEL})\n'
                 f'{REGION_LABELS[region]}\n'
...
# ============ README.txt ============
regions_done = [r for r in REGIONS if get_cols(r, df)]
readme = f"""Pixel Statistics Confound Analysis — ROC Curves
==================================================
Method: {METHOD_LABEL}
Regions: {', '.join(regions_done)}
...
Input CSV: {os.path.basename(args.input)}
