#!/usr/bin/env python3
"""Same as save_features_all_plates.py, but applies PER-CROP z-score
   normalization before computing stats. Column names are identical to
   the original CSV so existing plot scripts work unchanged.

   z-score: (pixel - crop_mean) / crop_std → forces mean=0, std=1 per crop.
"""
import argparse
parser = argparse.ArgumentParser()
parser.add_argument('--plates', default='P1,P2,P3,P4,P5,P6',
                    help='Comma-separated plate names')
parser.add_argument('--output', default='all_plates_features_zscore.csv',
...
def compute_stats(img, range_min=-3, range_max=3):
    """Stats on z-score normalized images (mean≈0, std≈1)."""
...
def process_plate(plate, base_dir, mapping, datatype):
    ...
    for well, paths in tqdm(wells.items(), desc=f'{datatype} {plate}'):
        ...
        # Z-score normalize EACH region independently before computing stats
        imgs_z = {
            'full': zscore(img),
            'center1128': zscore(img_1128),
            'center224': zscore(img_224),
        }
        ...
        for region_name, region_img in imgs_z.items():
            # Save with same column names as original CSV
            stats = compute_stats(region_img)
            for k, v in stats.items():
                entry[f'{region_name}_raw_{k}'] = v
                entry[f'{region_name}_mp_{k}'] = v  # both raw and mp are z-scored (preprocessing is identity)
...
print(f"  Features use identical column names as original CSV")
print(f"  But values are computed AFTER per-crop z-score normalization")
