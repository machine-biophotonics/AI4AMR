...
class MAECropDataset(Dataset):
    """Single-crop dataset for Masked Autoencoder.

    Each epoch selects a random 224×224 crop position per image from the
    full image (2720×2720). Uses tifffile.memmap to read only the crop region
    from disk (~100KB per image, not the full 14.1MB).

    Total 6.2M possible crop positions per image — effectively infinite
    augmentation. Per-epoch positions are deterministic (seeded by epoch number)
    for reproducibility.
    """
    def __init__(
        self,
        ...
        self.seed = seed
        self.epoch = epoch
        self.min_fg_patches = min_fg_patches
        ...
        w, h = sample_img.size
        if w != h:
            raise ValueError(f"Expected square images, got {w}x{h}")
        self.image_size = w

        # 144 grid positions (12×12), evenly spaced
        stride = (w - crop_size) // (grid_size - 1)
        self.positions = [(j * stride, i * stride) for i in range(grid_size) for j in range(grid_size)]
    ...
    def set_epoch(self, epoch: int) -> None:
        """Cycle through 144 grid positions, resetting every 144 epochs."""
        self.epoch = epoch
        ...
        if not self.augment:
            ...
            self.epoch_centers = {i: (center, center) for i in range(num_imgs)}
            return
    ...
    def _load_image(self, img_path: str) -> np.ndarray:
        try:
            import tifffile
            return tifffile.memmap(img_path)
        except ImportError:
    ...
    def _has_foreground(self, crop_01: np.ndarray) -> bool:
        p90 = np.quantile(crop_01, 0.90)
        return (crop_01 > p90).mean() > 0

    def __getitem__(self, idx: int) -> torch.Tensor:
        mmap = self._load_image(self.image_paths[idx])
        ...
        if self.min_fg_patches > 0 and not self._has_foreground(crop):
