...
class GrayscaleMixedCropDataset(Dataset):
    ...
    def __init__(self, image_paths, labels, crop_size=224, grid_size=12, augment=True, seed=42, epoch=0):
        ...
        transforms_list = [
            ...
            # Affine transformations (translate, scale, rotate) - reduced probability
        ...
        if crop_size >= 512:
            transforms_list.append(
                A.RandomResizedCrop(size=(crop_size, crop_size), scale=(0.8, 1.0), ratio=(0.9, 1.1), p=0.2)
            )
        
        transforms_list.extend([
            # === LIGHTING & CONTRAST (grayscale-appropriate) ===
            # CLAHE - enhances local contrast (great for microscopy) - reduced
            A.CLAHE(clip_limit=4.0, tile_grid_size=(8, 8), p=0.2),
            # Random brightness/contrast - reduced
            A.RandomBrightnessContrast(brightness_limit=0.15, contrast_limit=0.15, p=0.2),
            # Random gamma - reduced
            A.RandomGamma(gamma_limit=(85, 115), p=0.15),
            # Histogram equalization (global contrast) - reduced
            A.Equalize(p=0.05),
            ...
            # Colony deformation and camera angle variations - reduced probability
            A.SomeOf([
            ...
            # === NOISE & BLUR (reduced) ===
            # Sensor noise and out-of-focus blur - reduced probability
            A.SomeOf([
            ...
            # === PIXEL DROPOUT ===
            # Random pixel dropout - reduced
            A.PixelDropout(dropout_prob=0.03, drop_value=0, p=0.1),
            
            # === NOISE ARTIFACTS (reduced) ===
            # Additional grayscale-compatible noise augmentations - reduced
            ...
            # Image compression and dropout - reduced
...
def get_combined_weights(plates, labels):
    """Compute class weights only (domain weights disabled by default)"""
    weights = []
    for plate, label in zip(plates, labels):
        class_w = class_weights[label].item()
        # Domain weights disabled - all plates treated equally
...
class SAM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, rho=0.05, **kwargs):
        ...
        if isinstance(base_optimizer, torch.optim.Optimizer):
            ...
            # Need to add rho to each param group from base optimizer
...
if args.test_only:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (images, labels) in enumerate(tqdm(train_loader, desc=f'Epoch {epoch}', leave=False)):
            ...
            # Get plate info for this batch (using index to get from train_plates)
