...
Z1_N_TASKS = 6         # Z1 output heads (1 per ESKAPE species)
        ...
        if self.ensemble is not None:
            try:
                ...
                if valid_mask[0]:
                    ...
                    result["activity"] = float(np.mean(per_species))
    ...
    def predict_approx_batched(self, smiles_list: List[str]) -> List[Dict[str, float]]:
        ...
        if ensemble_smiles and mole_embs:
            try:
                ...
                _first_acts = [float(p) for p in batched_preds[:5]]
                if self.bb_mole_cache:
                    print(f"    [debug] predict_approx_batched: {len(ensemble_smiles)} candidates, "
                          f"first 5 activities={_first_acts}")
...
class ChempropMolEModel(nn.Module):
    ...
    def __init__(self, hidden_dim: int = HIDDEN_DIM, depth: int = DEPTH,
        ...
        self.norm = nn.LayerNorm(hidden_dim + self.mole_dim)
...
class RLModels:
    ...
    Z1 (activity): 6-head output (1 per ESKAPE species).
    Z2 (log solubility): scalar output.
    ...
    def __init__(self, hidden_dim: int = HIDDEN_DIM, depth: int = DEPTH):
        ...
        self.z2 = ChempropMolEModel(hidden_dim, depth, n_tasks=1).to(DEVICE)
    ...
    def predict_values_batched(self, bb_lists: List[List[str]]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Z1 returns 6-d per-species logits collapsed to scalar via geometric mean
        (broad-spectrum activity). Z2 returns scalar log solubility.
        ...
        with torch.no_grad():
            self.z1.eval()
            self.z2.eval()
            z1_logits = self.z1.forward_batched(bb_lists)  # (N, 6)
            z2_vals = self.z2.forward_batched(bb_lists).squeeze(-1)  # (N,)
            # Collapse 6 heads to scalar: geometric mean of sigmoid probabilities
            z1_probs = torch.sigmoid(z1_logits)  # (N, 6)
            z1_vals = torch.exp(torch.mean(torch.log(z1_probs.clamp(min=1e-6)), dim=1))  # (N,)
    ...
    def train_step_batched(self, bb_lists: List[List[str]],
        ...
        Args:
            ...
            z1_labels: Per-species activity targets, tensor (N, 6).
        ...
        z1_pred = self.z1.forward_batched(bb_lists)  # (N, 6)
        z2_pred = self.z2.forward_batched(bb_lists).squeeze(-1)  # (N,)

        loss_z1 = F.mse_loss(z1_pred, z1_labels)
...
class Node:
    ...
    z1_target is a 6-element list of per-species activity scores.
...
class RLGenerator:
    ...
    def __init__(
        ...
        self.models.set_caches(self.mole_cache, self.gnn_cache)
    ...
    def _init_from_pretrained(model: nn.Module, model_dir: Path):
        """Initialize GNN backbone from pretrained Chemprop checkpoint (paper Section 2.2.5).

        Loads message_passing weights into the RL value model.
        FFN and LayerNorm remain randomly initialized (trained via RL).
        """
        ...
        try:
            ...
            msg_passing_keys = {k: v for k, v in sd.items() if k.startswith("message_passing")}
            missing, unexpected = model.load_state_dict(msg_passing_keys, strict=False)
            loaded = len(msg_passing_keys)
            skipped_ffn = len([k for k in sd if k.startswith("predictor")])
            print(f"  Loaded {loaded} GNN layers from {model_dir.name} "
                  f"(skipped {skipped_ffn} FFN layers, {len(missing)} missing RL-only keys)")
        except Exception as e:
    ...
    def _single_rollout(self, rollout_id: int) -> Optional[Dict]:
        ...
        # Per-species scores as 6-d Z1 target (one head per ESKAPE species).
        # The RL model learns to predict individual species probabilities,
        # enabling species-level analysis post-hoc. Broad-spectrum selection
        # uses geometric mean of sigmoid outputs at inference time.
        if final_node.activity > 0 and final_node.per_species:
            z1_target = list(final_node.per_species.values())
            for node in rollout_nodes:
