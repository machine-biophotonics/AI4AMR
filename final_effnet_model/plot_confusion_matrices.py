...
def main():
    # Load predictions - use test set if full not available
    ...
    if not os.path.exists(csv_path):
        ...
        print(f"crop_predictions_full.csv not found, using crop_predictions_test.csv")
    ...
    for well, group in tqdm(image_df.groupby('well'), desc="Aggregating wells"):
        ...
        for _, row in group.iterrows():
            ...
            for p in img_crops['probs_json'].values:
                img_probs += np.array(json.loads(p)) / len(img_crops)
