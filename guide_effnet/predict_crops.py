...
def main():
    parser = argparse.ArgumentParser(description='Predict crops for guide generalization test images')
    parser.add_argument('--crop_size', type=int, default=224, help='Crop size (default: 224)')
    parser.add_argument('--grid_size', type=int, default=12, help='Grid size for crops (default: 12x12 = 144 crops)')
    ...
    parser.add_argument('--output_csv', type=str, default=os.path.join(SCRIPT_DIR, 'guide3_crop_predictions.csv'),
    ...
    parser.add_argument('--plates', type=str, nargs='+', default=['P1','P2','P3','P4','P5','P6'],
                        help='List of plates to process (default: all plates)')
    ...
    # Get all tif files from specified plates
    image_paths = []
    if args.image_dir:
        # Single directory mode
        image_paths = sorted(Path(args.image_dir).glob('*.tif'))
    else:
        # Multi-plate mode - get images from all specified plates
        base_dir = os.path.dirname(SCRIPT_DIR)  # Parent of guide_effnet
        for plate in args.plates:
            plate_dir = os.path.join(base_dir, plate)
            if os.path.exists(plate_dir):
                plate_images = sorted(Path(plate_dir).glob('*.tif'))
                print(f"Found {len(plate_images)} images in {plate}")
                image_paths.extend(plate_images)
            else:
                print(f"Warning: Plate directory {plate_dir} not found")
    
    if args.max_images:
        image_paths = image_paths[:args.max_images]
    
    print(f"Processing {len(image_paths)} images from plates: {args.plates}")
