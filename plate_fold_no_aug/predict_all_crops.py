#!/usr/bin/env python3
"""
Predict all 144 crops for each test image in plate_fold_no_aug experiment.
Tests all 6 folds (each fold: train on 4 plates, test on 1).
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Predict all crops for plate_fold_no_aug')
    ...
    for fold in folds_to_run:
        ...
        output_csv: str = os.path.join(output_dir, f'predictions_all_crops.csv')
        ...
        img_output_csv: str = os.path.join(output_dir, f'image_predictions_all_crops.csv')
        ...
        print(f"\n{'='*60}")
        print(f"Done! Fold {fold}, test plate {test_plate}")
        print(f"{'='*60}")
    ...
    for m in all_fold_metrics:
        print(f"Fold {m.get('fold', 'N/A')}: Majority vote acc = {m.get('majority_vote_accuracy', 'N/A'):.4f}")
